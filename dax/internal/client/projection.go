...
func (in *itemNode) toAttribute() *dynamodb.AttributeValue {
	if in == nil {
		return nil
	}
	if in.value != nil {
	...
	if len(in.children) != 0 {
		...
		if keys[0].index < 0 {
			val.M = make(map[string]*dynamodb.AttributeValue, len(keys))
			for k, v := range m {
				val.M[k.name] = v.toAttribute()
			}
		} else {
			...
			val.L = make([]*dynamodb.AttributeValue, 0, len(keys))
			for _, k := range keys {
				val.L = append(val.L, m[k].toAttribute())
			}
		}
	}
	return &val
...
func (ib *itemBuilder) insert(path documentPath, value *dynamodb.AttributeValue) {
	if ib.root == nil {
...
func (ib *itemBuilder) toItem() map[string]*dynamodb.AttributeValue {
...
func (ib *itemBuilder) insertNode(node *itemNode, elements []documentPathElement, value *dynamodb.AttributeValue) {
	if len(elements) == 0 {
		node.value = value
		return
	}
